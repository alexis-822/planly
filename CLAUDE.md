...
### 2026-09-18 (suite) — Faux prix, excuses publiées, horaires illisibles
- **Prix du Casino JOA : faux.** La fiche affichait 8 € adulte / 5 € enfant, tarifs venant de `joa.fr/casinos/`**`fouras`**`/loisirs/cinema` — le cinéma du casino de Fouras, à 150 km. Le garde-fou sur le choix des URL (16/09) n'avait jamais purgé les données déjà stockées. `pois.js` et `output_global.json` reviennent à « entrée libre, les jeux sont payants » ; le badge « € Petit budget », calculé sur ce faux prix, s'aligne sur l'autre casino. **Leçon : un correctif de collecte ne répare pas le passé — il faut purger.** Sauvegardes : `scratchpad/pois-avant-joa-18sept.js`, `output_global-avant-joa-18sept.json`
- **Brétignolles publiait les excuses de l'extracteur** : « Aucune information n'est présente dans les pages fournies », « Données insuffisantes pour compléter ce formulaire ». Deux causes. **Scraper** : `marchesdefrance.fr` (Fédération Nationale des Marchés) était retenu comme site officiel — un annuaire national, comme `restaurants-de-france` ; il rejoint `AGGREGATORS` avec `marches-de-france`, `jours-de-marche`, `marchesdenoel`, `tourisme-vendee`, pour que le pipeline retombe sur **le site de la commune**. **App** : `_sansAveux` les écarte quoi qu'il arrive — sur 88 fiches il supprime exactement 3 points, tous sur Brétignolles, tous de vraies excuses (**zéro faux positif, mesuré**)
- **`market_days` est une LISTE** (4 lieux, 4 calendriers, saisons comprises à Brétignolles). Le code faisait `String()` dessus → chaîne à virgules tronquée à 14 signes → « Place des Hall ». Chaque calendrier devient une ligne (lieu à gauche, jours à droite) ; `_joursMarche` résume le bandeau en « Mar · Ven · Dim » ou « 7j/7 »
- **Horaires : trois endroits les rendaient**, dont deux en bloc de texte dense. `_horairesEnLignes` (partagé) découpe en une phrase par ligne, « quoi » à gauche et « quand » à droite. **Le découpage exige l'espace AVANT le deux-points** (forme typographique française) : sans lui, « Lundi 18:30-22:30 » se coupait en « Lundi 18 » / « 30-22:30 » et UMI Sushi perdait son service du midi. La ligne d'état ne s'affiche plus quand elle n'a rien à dire (`_ficheEtat` renvoie `txt:''`). `_mtMarketBlock` ne rend les horaires que si Google occupe déjà le bloc — sinon Sel de l'Île perdait sa cabane de vente de juin à septembre, ou Arago les affichait deux fois
- **Troncatures en plein mot** : `_tronque` coupe au dernier espace et ajoute une ellipse. Deux coupes s'appliquaient **après** `_escHtml` et pouvaient trancher une entité (`&amp;` → `&am`)
- **Piège de nommage à retenir** : `_court` était déjà pris par une `var` du bloc plage, **dans la même portée de fonction** (`_renderPoiBody`). Un `var` hissé masque une fonction globale du même nom → `_court is not a function` sur les 21 fiches de la famille Manger. D'où `_tronque`. **Et la leçon d'outillage : `capture_fiches.js` écrit `Fiches rendues : X · en erreur : Y` — ne jamais tronquer cette ligne avec `head`/`tail`, elle portait le diagnostic exact**
- **Débordement flexbox** : sans `min-width:0`, un élément flex refuse de descendre sous la largeur de son texte — « tous les jours 7h30 à 13h » sortait de l'écran par la droite sur Arago

### 2026-09-18 (fin) — Intégrité des données, cœur, vue web, tri par distance
- **Trois choses inventées, retirées.** Après avoir purgé le faux prix du JOA, j'avais recopié le `budgetBadge` de **l'autre casino** (« €€ Modéré ») : rien ne le justifiait → « Prix non communiqué », formulation déjà employée 6 fois. `output_global.json` portait `free_entry:true`, `adult:0`, un `source_label:"site officiel"` et un **`verified_at:2026-09-18`** alors que je n'avais lu aucune page du JOA : **une fausse provenance rend une invention crédible** — tous ces champs passent à `null`, seule reste une note de purge. Et « 7j/7 » sur Brétignolles était **ma synthèse** de 4 calendriers saisonniers affichée comme un fait : `_joursMarche` se tait quand la liste a plusieurs entrées
- **Troncatures supprimées là où elles n'avaient pas lieu d'être** : ces valeurs deviennent des **lignes**, pas des colonnes de bandeau, et `_frRow` passe un texte long en précision. 17 champs retrouvent leur texte entier (« boutique de produits de la ruche », « Vue imprenable sur la baie… »). `_tronque` ne sert plus qu'aux badges de largeur fixe
- **Cœur absent depuis la recherche** : **trois** gabarits rebâtissaient une carte `.lcard` à la main sans le bouton. Mesure du trajet réel, en cliquant : 97 cartes pour 84 cœurs avant, **96 pour 96** après, à chaque étape
- **La ville en tête de fiche** (`_ficheHeader`) : l'écran de décision ne disait pas *où*, alors que les cartes de liste l'affichent depuis toujours
- **Ânes Passions, fermée** : retirée de `pois.js` (84 → 83) et marquée `permanently_closed` dans `output_global.json` — données conservées, publication arrêtée. **`inject_pois.py` ne filtrait que sur le taux de remplissage** : un lieu fermé restait complet à 100 % et serait revenu à la régénération
- **Liens externes : vue web interne** (`.wv`, `wvOuvrir`/`wvFermer`). `window.open` faisait sortir de l'app ; un panneau s'ouvre par-dessus la fiche, flèche Retour, la fiche attend derrière. **Mesuré sur les 31 sites de lieux : 22 acceptent le cadre, 9 le refusent** (X-Frame-Options / frame-ancestors) → liste d'exclusion, plus Google/Facebook/Instagram/Apple et l'itinéraire, qui doit partir vers l'app de navigation
- **Recherche : tri par distance** à pertinence voisine (palier de 10 ; un palier de 5 reléguait la Grande Plage du Remblai à 1,3 km derrière une plage à 51 km). Les cartes affichent **km + temps de route** (`_srchTrajet`) : plus besoin d'ouvrir chaque fiche
- **Trois pièges de cette fin de session** : (1) **ne jamais deviner un domaine** — j'avais écrit `rooftopventura.fr`, `ofunpark.com`, `lacabaneduptitgas.fr`, aucun n'existe ; les vrais sont dans les données ; (2) **un délai de repli trop court est pire que long** — 2,5 s coupaient le chargement d'Explora Parc avant la fin, le panneau se fermait et `window.open` était bloqué faute de geste utilisateur → 6 s ; (3) **un symbole absent du sprite donne un bouton invisible** — `i-arrow-left` n'existe pas, `controle.js` l'a dit (« absents : i-arrow-left »), le sprite a `i-chev-l`
- **OÙ ON EN EST (fin de session 18/09)** — tout est poussé et en ligne. **Reste ouvert, par ordre d'intérêt :**
  1. **Horaires structurés pour 14 lieux** (décidé avec l'utilisateur, pas lancé) : relancer la collecte Google pour La Cotriade, UMI Sushi, Marché Arago, Vignobles Mourat, Famille Mary, Circuit Mecamax, Atlantic Kart System, AxeYon Paintball, O'Fun Park, Cinéma Le Grand Palace, Bar Rooftop Ventura, Casino JOA, Aqualonne. Objectif : passer de 22 à 36 fiches capables d'un « Ouvert/Fermé » fiable. **Attention** : `scraper_missing.py --poi` relance **tout le pipeline officiel** du lieu, pas seulement les horaires — vérifier qu'il n'écrase pas des champs corrects avant de dépenser l'API (~0,25 $)
  2. **Fiches Patrimoine maigres** (Tour d'Arundel, Abbaye de Maillezais, Fort Saint-Nicolas, Prieuré Saint-Nicolas, Église de Brem, Les Salines, L'Étoile de Mer, Bikini Beach) : leurs données sont réellement vides depuis la suppression des mots-clés. Ne pas les remplir de mots creux — il faut une vraie collecte
  3. **Halles de La Chaume** : aucune source, aucun horaire, aucun jour de marché. Le correctif `AGGREGATORS` (marchesdefrance et consorts) devrait permettre au pipeline de retomber sur le site de la commune — non retesté
  4. Second niveau de fond (`--ground`/`--sheet`, jetons posés, 0 usage) ; barre de prix épinglée (`.bs-price-pin`, CSS présent, aucun rendu) ; ~250 lignes de CSS `bs-*` mort + `_renderTypeBlock` jamais appelé
- **Pastille d'état** restaurée sur les 14 fiches à horaires longs, rendues muettes la veille. Elles disent « Horaires · voir plus bas » : **seules 22 fiches sur 83 ont des horaires structurés** permettant un « Ouvert/Fermé » fiable ; 10 plages affichent « Surveillée », 38 « Accès libre ». Annoncer « Ouvert » à partir d'une phrase libre serait une devinette — **un vert faux est pire que pas de vert**
